...
addon_data.target.UpdateVisualsOnUpdate = function()
    ...
    if settings.enabled and UnitExists("target") then
        ...
        if main_width == settings.width or main_width == 0.001 then
        ...
        if addon_data.target.has_offhand and settings.show_offhand then
            ...
            if off_width == settings.width or off_width == 0.001 then
...
addon_data.target.UpdateVisualsOnSettingsChange = function()
    ...
    if settings.enabled then
        ...
        frame.main_bar:SetTexture(addon_data.utils.GetBarTexture())
		...
		frame.main_left_text:SetFont(STANDARD_TEXT_FONT, settings.fontsize)
		...
		frame.main_right_text:SetFont(STANDARD_TEXT_FONT, settings.fontsize)
		
        frame.off_bar:SetPoint("BOTTOMLEFT", 0, 0)
        frame.off_bar:SetHeight(settings.height)
        frame.off_bar:SetTexture(addon_data.utils.GetBarTexture())
		...
		frame.off_left_text:SetFont(STANDARD_TEXT_FONT, settings.fontsize)
		...
		frame.off_right_text:SetFont(STANDARD_TEXT_FONT, settings.fontsize)
